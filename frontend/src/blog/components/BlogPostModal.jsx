...
// ─── CTA block ─────────────────────────────────────────────────────────────
// Ведёт читателя от контента к бизнесу: строительству скейтпарков.
// Источник текста: post.cta (задан вручную в админке) → рубрика.ctaDefault → общий fallback.
// post.cta === false отключает блок для конкретного поста (см. resolveCta).

const CtaBlock = memo(({ cta }) => {
  if (!cta) return null
  return (
    <div className="relative overflow-hidden border border-[#ff1493]/30 bg-gradient-to-br from-[#ff1493]/10 via-[#1a1a1a] to-[#1a1a1a] p-4 md:p-5">
      <div className="absolute -right-6 -top-6 text-[90px] leading-none text-[#ff1493]/10 select-none pointer-events-none">
        🏗️
      </div>
      <p className="font-futura font-black text-white text-base md:text-lg uppercase tracking-tight mb-1.5 relative">
        {cta.title}
      </p>
      {cta.text && (
        <p className="font-futura text-white/60 text-[13px] leading-relaxed mb-3 max-w-[80%] relative">
          {cta.text}
        </p>
      )}
      <a
        href={cta.buttonUrl || "/contact"}
        className="inline-flex items-center gap-2 bg-[#ff1493] text-white font-futura font-bold text-[12px] uppercase tracking-wide px-4 py-2 hover:bg-[#ff69b4] transition-colors relative"
      >
        {cta.buttonLabel || "Оставить заявку"}
        <span>→</span>
      </a>
    </div>
  )
        ...
        <div className="relative w-full h-full md:max-w-[95vw] md:max-h-[95vh] md:h-auto bg-[#0a0a0a] border-0 md:border md:border-white/[0.1] shadow-2xl flex flex-col overflow-hidden">
          ...
          <div className="flex-shrink-0 bg-[#1a1a1a] border-b border-white/[0.08]">
            <div className="h-12 md:h-14 px-3 md:px-4 flex items-center justify-between gap-3">
              ...
              {/* Title (hidden on mobile) */}
              <div className="hidden md:flex flex-1 min-w-0 px-3 items-center gap-2">
                {section && (
                  <span
                    className="text-[10px] font-futura font-black uppercase tracking-wider px-1.5 py-0.5 flex-shrink-0"
            ...
            <div className="flex flex-col md:flex-row h-full">
              ...
              <div className="right-scroll flex flex-col flex-1 min-w-0 overflow-y-auto">
                <div className="p-4 md:p-5 flex flex-col gap-4 md:gap-5">
                  ...
                  {/* ── Article header ── */}
                  <div className="pb-3 md:pb-4 border-b border-white/10">
                    {/* Rubric + Tags + UPDATED badge */}
                    <div className="flex flex-wrap gap-1.5 mb-3 items-center">
                      {section && (
                        <span
                          className="text-[9px] font-black uppercase tracking-[0.18em] px-2 py-1 font-futura"
                      ...
                      {post.tags?.map(t => (
                        <span
                          key={t}
                          className="bg-white/10 text-white/70 text-[9px] font-black uppercase tracking-[0.18em] px-2 py-1 font-futura"
                        >
                          {t}
                        </span>
                      ))}
