...
import { useEffect, useState, useRef, useCallback } from "react"
import { SECTIONS, getSection } from "./section"
...
function EmojiPicker({ onSelect, onClose, anchor = "bottom" }) {
  ...
  return (
    <div ref={wrapRef}
      className={`absolute right-0 ${posClass} z-[100] w-[320px] bg-[#161616] border border-white/10 rounded-xl shadow-2xl flex flex-col overflow-hidden`}
      onClick={e => e.stopPropagation()}>

      <div className="flex items-center justify-between px-3 py-2 border-b border-white/10 bg-[#1c1c1c]">
        <span className="text-[11px] font-bold uppercase tracking-widest text-white/30 font-futura">Эмодзи</span>
        <button onClick={onClose} className="text-white/20 hover:text-white/70 text-base leading-none transition-colors cursor-pointer">✕</button>
      </div>
      ...
      <div className="px-3 pt-2 pb-1.5 border-b border-white/10">
        <div className="relative">
          ...
          <input ref={searchRef} type="text" value={search}
            onChange={e => setSearch(e.target.value)}
            placeholder="Поиск..."
            className="w-full text-sm pl-8 pr-8 py-1.5 bg-[#0f0f0f] border border-white/10 text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-[#ff1493] focus:border-transparent font-futura placeholder:text-white/20"
          />
          {search && (
            <button onClick={() => setSearch("")}
              className="absolute right-2 top-1/2 -translate-y-1/2 text-white/20 hover:text-white/50 text-xs cursor-pointer">✕</button>
          )}
        </div>
      </div>

      {!search && (
        <div className="flex overflow-x-auto bg-[#1c1c1c] border-b border-white/10" style={{ scrollbarWidth: "none" }}>
          {cats.map(cat => (
            <button key={cat.id} onClick={() => {
              setActive(cat.id)
              bodyRef.current?.querySelector(`[data-cat="${cat.id}"]`)?.scrollIntoView({ behavior: "smooth", block: "start" })
              ...
              title={cat.label}
              className={`flex-shrink-0 w-9 h-8 flex items-center justify-center text-base transition-all cursor-pointer border-b-2 ${
                activeCat === cat.id ? "border-[#ff1493] bg-[#161616]" : "border-transparent hover:bg-white/[0.04]"
              }`}>
              <span className="text-[16px] leading-none">{cat.icon}</span>
            </button>
          ))}
      ...
      <div ref={bodyRef}
        ...
        {display.map(cat => (
          <div key={cat.id} data-cat={cat.id}>
            <div className="sticky top-0 z-10 px-3 py-1 bg-[#161616]/95 backdrop-blur-sm border-b border-white/[0.04]">
              <span className="text-[9px] font-bold uppercase tracking-widest text-white/25 font-futura">{cat.label}</span>
            </div>
            <div className="grid p-1.5" style={{ gridTemplateColumns: "repeat(8, 1fr)" }}>
              {cat.emojis.length === 0
                ? <div className="col-span-8 py-5 text-center text-white/20 text-[11px] uppercase tracking-widest font-futura">пусто</div>
                : cat.emojis.map((emoji, i) => (
                    <button key={`${emoji}${i}`} onClick={() => pick(emoji)}
                      className="w-9 h-9 flex items-center justify-center text-[19px] hover:bg-white/[0.06] active:scale-90 transition-all rounded-md cursor-pointer">
                      {emoji}
                    </button>
                  ))
              }
      ...
      <div className="px-3 py-1.5 bg-[#1c1c1c] border-t border-white/10 flex items-center justify-between">
        <span className="text-white/25 text-[9px] font-futura">
          {recent.length > 0 ? `Недавние: ${recent.slice(0, 5).join(" ")}` : "Выберите эмодзи"}
        </span>
        {recent.length > 0 && (
          <button onClick={() => { saveRecent([]); setRecent([]) }}
            className="text-white/25 hover:text-white/60 text-[9px] cursor-pointer transition-colors font-futura">
            Очистить
          </button>
        )}
      </div>
  ...
  return (
    <div className="relative flex-shrink-0">
      <button type="button"
        ...
        title="Вставить эмодзи"
        className={`h-9 w-9 flex items-center justify-center border rounded-lg text-base transition-all cursor-pointer font-futura ${
          open ? "bg-[#ff1493] border-[#ff1493] text-white" : "border-white/15 text-white/40 hover:border-white/40 hover:text-white/80"
        }`}>
        😊
      </button>
      {open && (
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between">
        <label className="text-[11px] font-bold text-white/40 uppercase tracking-wide font-futura">{label}</label>
        {hint && <span className="text-[10px] text-white/20 font-futura">{hint}</span>}
      </div>
...
const inputCls = "w-full bg-[#111] border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder:text-white/20 focus:outline-none focus:ring-2 focus:ring-[#ff1493] focus:border-transparent font-futura transition-all"

// ─── Rubric (section) selector — визуальные плитки, не dropdown ───────────
      ...
      {SECTIONS.map(s => {
        ...
        return (
          <button key={s.id} type="button" onClick={() => onChange(s.id)}
            ...
            <span className="text-[12px] font-futura font-bold" style={{ color: active ? s.color : "rgba(255,255,255,0.6)" }}>
              {s.label}
            </span>
          </button>
        )
...
// ─── CTA editor — три режима: рубрика / свой / скрыт ──────────────────────

function CtaEditor({ section, cta, onChange }) {
  const mode = cta === false ? "hidden" : (cta && typeof cta === "object") ? "custom" : "default"
  const sectionDefault = section?.ctaDefault

  function setMode(next) {
    if (next === "default") onChange(undefined)
    if (next === "hidden") onChange(false)
    if (next === "custom") onChange({
      title: sectionDefault?.title || "",
      text: sectionDefault?.text || "",
      buttonLabel: sectionDefault?.buttonLabel || "Оставить заявку",
      buttonUrl: sectionDefault?.buttonUrl || "/contact",
    })
  }

  const customCta = mode === "custom" ? cta : null

  return (
    <div className="border border-white/10 rounded-lg overflow-hidden">
      <div className="bg-[#111] px-3 py-2 flex items-center gap-2 border-b border-white/10">
        <span className="text-[11px] font-bold text-white/40 uppercase tracking-wide font-futura flex-1">
          Призыв к действию (CTA)
        </span>
        <div className="flex gap-1">
          {[
            { id: "default", label: "Рубрика" },
            { id: "custom",  label: "Свой" },
            { id: "hidden",  label: "Скрыть" },
          ].map(opt => (
            <button key={opt.id} type="button" onClick={() => setMode(opt.id)}
              className={`px-2 py-1 rounded text-[10px] font-futura font-bold uppercase tracking-wide transition-all cursor-pointer ${
                mode === opt.id ? "bg-[#ff1493] text-white" : "text-white/30 hover:text-white/60"
              }`}>
              {opt.label}
            </button>
          ))}
      ...
      <div className="p-3">
        {mode === "default" && (
          sectionDefault ? (
            <div className="text-[12px] font-futura text-white/40">
              Будет показан CTA рубрики «{section.label}»: <span className="text-white/60">{sectionDefault.title}</span>
            </div>
          ) : (
            <div className="text-[12px] font-futura text-white/25 italic">
              У этой рубрики нет CTA по умолчанию — блок не отображается
            </div>
          )
        )}
        ...
        {mode === "hidden" && (
          <div className="text-[12px] font-futura text-white/25 italic">
            CTA скрыт для этого поста
          </div>
        )}

        {mode === "custom" && customCta && (
          <div className="space-y-2">
            <input className={inputCls} placeholder="Заголовок CTA"
              value={customCta.title}
              onChange={e => onChange({ ...customCta, title: e.target.value })} />
            <textarea className={inputCls + " resize-none"} rows="2" placeholder="Текст CTA"
              value={customCta.text}
              onChange={e => onChange({ ...customCta, text: e.target.value })} />
            <div className="grid grid-cols-2 gap-2">
              <input className={inputCls} placeholder="Текст кнопки"
                value={customCta.buttonLabel}
                onChange={e => onChange({ ...customCta, buttonLabel: e.target.value })} />
              <input className={inputCls} placeholder="Ссылка кнопки"
                value={customCta.buttonUrl}
                onChange={e => onChange({ ...customCta, buttonUrl: e.target.value })} />
            </div>
          </div>
        )}
      </div>
...
// ─── Drawer (боковая панель редактирования вместо центрального модала) ────
// Список постов остаётся видимым слева — быстрее переключаться между постами при модерации.
...
function Drawer({ title, badge, onClose, children }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      />
      <div
        className={`relative w-full max-w-2xl h-full bg-[#0d0d0d] border-l border-white/10 shadow-2xl flex flex-col transition-transform duration-200 ${
          show ? "translate-x-0" : "translate-x-full"
        }`}
      >
        <div className="flex-shrink-0 flex items-center justify-between px-5 py-4 border-b border-white/10 bg-[#141414]">
          <div>
            <h2 className="text-lg font-bold text-white font-futura">{title}</h2>
            {badge}
          </div>
          <button onClick={handleClose} className="w-8 h-8 flex items-center justify-center text-white/40 hover:text-white hover:bg-white/10 rounded-lg text-xl leading-none transition-colors cursor-pointer">✕</button>
        </div>
...
function PostForm({ initial = {}, onSave, onCancel, onBump, loading }) {
  ...
  const hasVideo = !!(form.url || form.video || (form.videos && form.videos.length > 0))
  const currentSection = getSection(form.section)
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ── Рубрика — первое и обязательное решение ── */}
      <Field label="Рубрика *" hint="определяет CTA и раздел на сайте">
        <SectionSelect value={form.section} onChange={v => set("section", v)} />
      </Field>
      ...
      <Field label="Заголовок *">
        <div className="flex gap-1.5 items-center relative">
          <input
            ...
            placeholder="Заголовок поста"
          />
          <EmojiFieldButton value={form.title} onChange={v => set("title", v)} anchor="bottom" />
        </div>
      </Field>

      {/* ── Краткое описание ── */}
      <Field label="Краткое описание" hint="для превью в ленте">
        <div className="flex gap-1.5 items-center relative">
          <textarea
          ...
          />
          <EmojiFieldButton value={form.excerpt || ""} onChange={v => set("excerpt", v)} anchor="bottom" />
        </div>
      </Field>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <Field label="Автор" hint="необязательно">
          <input className={inputCls} placeholder="Имя автора"
      ...
      <Field label="Теги">
        {form.tags.length > 0 && (
          <div className="flex gap-1.5 flex-wrap mb-2">
            {form.tags.map(tag => (
              <span key={tag}
                className="flex items-center gap-1 px-2.5 py-0.5 bg-[#ff1493] text-white text-xs rounded-full font-futura">
                #{tag}
                <button type="button" onClick={() => set("tags", form.tags.filter(t => t !== tag))}
                  className="text-white/60 hover:text-white leading-none cursor-pointer">✕</button>
              </span>
            ))}
          </div>
        )}
        <div className="flex gap-1.5 flex-wrap mb-2">
          {TAGS.filter(t => !form.tags.includes(t)).map(tag => (
            <button key={tag} type="button" onClick={() => toggleTag(tag)}
              className="px-2.5 py-0.5 rounded-full text-xs border border-dashed border-white/15 text-white/40 hover:border-white/40 hover:text-white/80 transition-all cursor-pointer font-futura">
              +#{tag}
            </button>
          ))}
        </div>
      ...
      <Field label="Обложка / Фото">
        <div className="flex gap-2">
          <input
            ...
            placeholder="URL или загрузи файл" />
          <button type="button" onClick={() => fileRef.current.click()} disabled={uploading}
            className="px-3 py-2 border border-white/15 text-white/70 rounded-lg text-sm hover:bg-white/[0.06] transition whitespace-nowrap cursor-pointer font-futura disabled:opacity-50">
            {uploading ? "⏳" : "📎 Загрузить"}
          </button>
          <input ref={fileRef} type="file" accept="image/*,video/*" className="hidden"
            onChange={e => e.target.files[0] && uploadFile(e.target.files[0], "cover")} />
        </div>
        {form.cover && <img src={form.cover} className="mt-2 h-24 object-cover rounded-lg border border-white/10" alt="" />}
      </Field>
      ...
      <Field label={
        <span className="flex items-center gap-2">
          YouTube / Rumble URL
          {hasVideo && (
            <span className="inline-flex items-center gap-1 text-[10px] font-bold text-[#22c55e] bg-[#22c55e]/10 px-2 py-0.5 rounded-full normal-case">
              ✓ Видео есть
            </span>
          )}
        </span>
      ...
      {/* ── Дополнительные видео ── */}
      <Field label="Дополнительные видео" hint="mp4, YouTube, Rumble">
        <div className="space-y-1.5">
          {(form.videos || []).map((v, i) => (
            <div key={i} className="flex gap-2 items-center">
              ...
              <button type="button"
                onClick={() => set("videos", form.videos.filter((_, idx) => idx !== i))}
                className="px-3 py-2 border border-red-500/30 text-red-400 rounded-lg text-sm hover:bg-red-500/10 transition cursor-pointer font-futura">
                ✕
              </button>
            </div>
          ))}
        ...
        <div className="mt-2 flex gap-2">
          <button type="button"
            onClick={() => set("videos", [...(form.videos || []), ""])}
            className="px-3 py-2 border border-white/15 text-white/60 rounded-lg text-sm hover:bg-white/[0.06] transition cursor-pointer font-futura">
            + Добавить URL видео
          </button>
          <button type="button"
            onClick={() => videosRef.current.click()}
            disabled={uploadingExtra}
            className="px-3 py-2 border border-white/15 text-white/60 rounded-lg text-sm hover:bg-white/[0.06] transition whitespace-nowrap cursor-pointer disabled:opacity-50 font-futura">
            {uploadingExtra ? "⏳" : "📎 Загрузить"}
          </button>
          <input ref={videosRef} type="file" accept="video/*" className="hidden" multiple
            onChange={e => {
      ...
      {/* ── Дополнительные фото ── */}
      <Field label="Дополнительные фото" hint="стрелки меняют порядок">
        <div className="space-y-1.5">
          {(form.photos || []).map((p, i) => (
            <div key={i} className="flex gap-2 items-center">
              <div className="flex flex-col gap-0.5">
                <button type="button" onClick={() => movePhoto(i, -1)} disabled={i === 0}
                  className="w-6 h-4 flex items-center justify-center text-white/30 hover:text-white disabled:opacity-20 cursor-pointer text-[10px]">▲</button>
                <button type="button" onClick={() => movePhoto(i, 1)} disabled={i === form.photos.length - 1}
                  className="w-6 h-4 flex items-center justify-center text-white/30 hover:text-white disabled:opacity-20 cursor-pointer text-[10px]">▼</button>
              </div>
              {p && <img src={p} className="w-10 h-10 object-cover rounded flex-shrink-0 border border-white/10" alt="" onError={e => e.target.style.display = "none"} />}
              <input
                className={inputCls}
                value={p}
                onChange={e => {
      ...
      <Field label={
        ...
        <div className="flex justify-end mb-1">
          <EmojiFieldButton value={form.content || ""} onChange={v => set("content", v)} anchor="top" />
        </div>
        <textarea
      ...
      {/* ── CTA ── */}
      <Field label="CTA под постом">
        <CtaEditor section={currentSection} cta={form.cta} onChange={v => set("cta", v)} />
      </Field>

      {/* ── Actions ── */}
      <div className="flex gap-3 pt-2 sticky bottom-0 bg-[#0d0d0d] pb-1 -mx-5 px-5 border-t border-white/10 mt-6">
        <div className="flex gap-3 pt-4 w-full">
          <button onClick={() => onSave(form)} disabled={!form.title || loading}
            className="flex-1 py-2.5 bg-[#ff1493] text-white rounded-lg text-sm font-semibold hover:bg-[#ff69b4] transition disabled:opacity-40 font-futura">
            {loading ? "Сохранение..." : "Сохранить"}
          </button>
          {initial.id && onBump && (
            <button type="button" onClick={() => onBump(initial.id)}
              title="Поднять пост наверх ленты"
              className="px-4 py-2.5 border border-white/15 text-white/70 rounded-lg text-sm hover:bg-white hover:text-[#0d0d0d] hover:border-white transition whitespace-nowrap font-futura">
              ↑ Поднять
            </button>
          )}
          <button onClick={onCancel}
            className="px-6 py-2.5 border border-white/15 text-white/60 rounded-lg text-sm hover:bg-white/[0.06] transition font-futura">
            Отмена
          </button>
        </div>
      </div>
...
function StatusPill({ status }) {
  const map = {
    pending:   { label: "⏳ На модерации", cls: "bg-amber-500/15 text-amber-400" },
    published: { label: "✓ Опубликовано",  cls: "bg-[#22c55e]/15 text-[#22c55e]" },
    rejected:  { label: "✕ Отклонено",      cls: "bg-red-500/15 text-red-400" },
    expired:   { label: "🕐 Истекло",       cls: "bg-white/10 text-white/40" },
  }
  const s = map[status] || { label: status, cls: "bg-white/10 text-white/40" }
  return <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full font-futura ${s.cls}`}>{s.label}</span>
}
...
function PostRow({ post, onEdit, onDelete, onApprove }) {
  ...
  return (
    <div
      className="flex items-center gap-4 p-4 border-b border-white/[0.06] last:border-0 hover:bg-white/[0.02] transition-colors border-l-2"
      style={{ borderLeftColor: section?.color || "transparent" }}
    >
      {post.cover ? (
        <img src={post.cover} alt="" className="w-16 h-16 object-cover rounded-lg flex-shrink-0" />
      ) : (
        <div className="w-16 h-16 rounded-lg flex-shrink-0 bg-white/[0.04] flex items-center justify-center text-white/10 text-xl">✦</div>
      )}
      <div className="flex-1 min-w-0">
        ...
        <h3 className="font-semibold text-white/90 truncate font-futura">{post.title}</h3>
        <p className="text-sm text-white/35 font-futura">{dateLabel} · {post.type}{post.author ? ` · ${post.author}` : ""}</p>
        {post.tags?.length > 0 && (
          <div className="flex gap-1.5 mt-1 flex-wrap">
            {post.tags.map(t => (
              <span key={t} className="text-[10px] bg-white/[0.06] text-white/50 px-2 py-0.5 rounded-full font-futura">
                {t}
              </span>
            ))}
          </div>
      ...
      <div className="flex gap-2 flex-shrink-0">
        {isPending && (
          <button onClick={() => onApprove(post.id)}
            className="px-3 py-1.5 bg-[#22c55e] text-white text-xs rounded-lg hover:bg-[#1ea34e] transition font-futura font-bold">
            ✓ Опубликовать
          </button>
        )}
        <button onClick={() => onEdit(post)}
          className="px-3 py-1.5 border border-white/15 text-white/70 text-xs rounded-lg hover:bg-white/[0.08] transition font-futura">
          Редактировать
        </button>
        <button onClick={handleDelete} disabled={deleting}
          className="px-3 py-1.5 border border-red-500/25 text-red-400 text-xs rounded-lg hover:bg-red-500/10 transition disabled:opacity-40 font-futura">
          {deleting ? "⏳" : "Удалить"}
        </button>
      </div>
    </div>
...
function ListingRow({ listing, onApprove, onReject, onDelete }) {
  ...
  return (
    <div className="flex items-start gap-4 p-4 border-b border-white/[0.06] last:border-0 hover:bg-white/[0.02] transition-colors">
      {listing.photos?.[0] ? (
        <img src={listing.photos[0]} alt="" className="w-20 h-20 object-cover rounded-lg flex-shrink-0" />
      ) : (
        <div className="w-20 h-20 rounded-lg flex-shrink-0 bg-white/[0.04] flex items-center justify-center text-white/10 text-xl">✦</div>
      )}
      <div className="flex-1 min-w-0">
        <h3 className="font-semibold text-white/90 font-futura">{listing.title}</h3>
        <p className="text-sm text-white/50 mt-1 font-futura">{listing.price} · {listing.category}</p>
        <div className="mt-1.5"><StatusPill status={listing.status} /></div>
        <p className="text-xs text-white/25 mt-1 font-futura">
      ...
      <div className="flex gap-2 flex-shrink-0">
        {listing.status === "pending" && (
          <>
            ...
            <button onClick={() => {
              ...
              className="px-3 py-1.5 bg-amber-600 text-white text-xs rounded-lg hover:bg-amber-700 transition font-futura font-bold">
              ✕ Reject
            </button>
          </>
        )}
        <button onClick={async () => {
          ...
          className="px-3 py-1.5 border border-red-500/25 text-red-400 text-xs rounded-lg hover:bg-red-500/10 transition font-futura">
          {deleting ? "⏳" : "Delete"}
        </button>
      </div>
    </div>
...
export default function AdminPage() {
  ...
  if (!authed) return (
    <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center relative overflow-hidden">
      ...
      <div className="relative bg-[#111] border border-white/10 p-8 rounded-2xl shadow-2xl w-full max-w-sm">
        <div className="w-10 h-10 bg-[#ff1493] rounded-lg flex items-center justify-center text-xl mb-4">🏗️</div>
        <h1 className="text-2xl font-bold text-white mb-1 font-futura">Admin</h1>
        <p className="text-white/30 text-sm mb-6 font-futura">Вход в панель управления</p>
        <input
          type="text"
          placeholder="Login"
          value={loginInput}
        ...
        />
        <button onClick={login}
          className="w-full py-2.5 bg-[#ff1493] text-white rounded-lg text-sm font-semibold hover:bg-[#ff69b4] transition font-futura">
          Войти
        </button>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex">
      ...
      {/* ══ SIDEBAR ══════════════════════════════════════════════════════ */}
      <aside className="w-[220px] flex-shrink-0 bg-[#111] border-r border-white/10 flex flex-col hidden md:flex">
        <div className="px-4 py-4 border-b border-white/10 flex items-center gap-2">
          <div className="w-8 h-8 bg-[#ff1493] rounded-lg flex items-center justify-center text-sm flex-shrink-0">🏗️</div>
          <span className="font-futura font-black text-white text-sm uppercase tracking-wide">Admin</span>
        </div>

        <nav className="flex-1 p-2 space-y-1">
          <button onClick={() => setTab("blog")}
            className={`w-full flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-futura font-semibold transition-all cursor-pointer relative ${
              tab === "blog" ? "bg-[#ff1493] text-white" : "text-white/50 hover:bg-white/[0.06] hover:text-white/80"
            }`}>
            📝 Блог
            <span className={`ml-auto text-xs ${tab === "blog" ? "opacity-70" : "opacity-30"}`}>{posts.length}</span>
            {pendingPostsCount > 0 && (
          ...
          <button onClick={() => setTab("market")}
            className={`w-full flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm font-futura font-semibold transition-all cursor-pointer relative ${
              tab === "market" ? "bg-[#ff1493] text-white" : "text-white/50 hover:bg-white/[0.06] hover:text-white/80"
            }`}>
            🛍️ Барахолка
            <span className={`ml-auto text-xs ${tab === "market" ? "opacity-70" : "opacity-30"}`}>{listings.length}</span>
            {pendingCount > 0 && (
              <span className="absolute -top-1 -right-1 w-4 h-4 bg-amber-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center">
                {pendingCount}
              </span>
        ...
        {/* Rubric quick nav (blog tab only) */}
        {tab === "blog" && (
          <div className="p-2 border-t border-white/10">
            <p className="px-3 py-1 text-[10px] font-futura font-bold uppercase tracking-widest text-white/25">Рубрики</p>
            <button onClick={() => setActiveSection("all")}
              className={`w-full flex items-center gap-2 px-3 py-1.5 rounded-lg text-[12px] font-futura transition-colors cursor-pointer ${
                activeSection === "all" ? "bg-white/[0.08] text-white" : "text-white/40 hover:text-white/70"
              }`}>
              ✦ Все
            </button>
            {SECTIONS.map(s => (
              <button key={s.id} onClick={() => setActiveSection(s.id)}
                className={`w-full flex items-center gap-2 px-3 py-1.5 rounded-lg text-[12px] font-futura transition-colors cursor-pointer ${
                  activeSection === s.id ? "bg-white/[0.08]" : "hover:text-white/70"
                }`}
                style={{ color: activeSection === s.id ? s.color : "rgba(255,255,255,0.4)" }}>
                {s.icon} {s.label}
              </button>
            ))}
          </div>
        ...
        <div className="p-2 border-t border-white/10">
          <button onClick={logout}
            className="w-full px-3 py-2 text-sm text-white/40 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition font-futura text-left cursor-pointer">
            ↩ Выйти
          </button>
        </div>
      </aside>
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        {/* Top bar */}
        <div className="bg-[#111] border-b border-white/10 px-5 py-3 flex items-center gap-3 flex-wrap">
          <h1 className="font-futura font-black text-white text-lg flex-shrink-0 md:hidden">
            {tab === "blog" ? "📝 Блог" : "🛍️ Барахолка"}
          </h1>

          {tab === "blog" ? (
            <input className={inputCls + " w-56"}
              placeholder="Поиск по заголовку..." value={search} onChange={e => setSearch(e.target.value)} />
          ) : (
            <>
              <input className={inputCls + " w-56"}
                placeholder="Поиск по названию..." value={lSearch} onChange={e => setLSearch(e.target.value)} />
                ...
                {[
                  ...
                  <button key={f.id} onClick={() => setLStatus(f.id)}
                    className={`px-3 py-1.5 rounded-full text-xs border transition-all cursor-pointer font-futura ${
                      lStatus === f.id ? "bg-white text-[#0a0a0a] border-white" : "border-white/15 text-white/50 hover:border-white/40"
                    }`}>
                    {f.label}
                  </button>
                ))}
          ...
          <div className="ml-auto flex gap-2">
            {tab === "blog" && (
              <button onClick={() => setCreating(true)}
                className="px-4 py-2 bg-[#ff1493] text-white rounded-lg text-sm font-semibold hover:bg-[#ff69b4] transition font-futura">
                + Новый пост
              </button>
            )}
            {tab === "market" && (
              <button onClick={loadListings} disabled={lLoading}
                className="px-4 py-2 border border-white/15 text-white/60 rounded-lg text-sm hover:bg-white/[0.06] transition disabled:opacity-40 font-futura">
                {lLoading ? "⏳" : "↻ Обновить"}
              </button>
            )}
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {tab === "blog" && (
            <div className="max-w-4xl mx-auto bg-[#111] rounded-xl border border-white/10 overflow-hidden shadow-sm">
              {loading ? (
                <div className="p-12 text-center text-white/30 font-futura">Загрузка...</div>
              ) : filteredPosts.length === 0 ? (
                <div className="p-12 text-center text-white/30 font-futura">Постов не найдено</div>
              ) : filteredPosts.map(post => (
                <PostRow key={post.id} post={post} onEdit={setEditing} onDelete={deletePost} onApprove={approvePost} />
              ))}
            </div>
          ...
          {tab === "market" && (
            <div className="max-w-4xl mx-auto bg-[#111] rounded-xl border border-white/10 overflow-hidden shadow-sm">
              {lLoading ? (
                ...
                <div className="p-12 text-center text-white/30 font-futura">
                  {lStatus === "pending" ? "Нет объявлений на модерации 🎉" : "Объявлений не найдено"}
                </div>
              ) : filteredListings.map(l => (
                <ListingRow key={l.id} listing={l}
      ...
      {/* Drawer редактирования — список остаётся видимым слева */}
      {editing && (
        <Drawer title="Редактировать пост" onClose={() => setEditing(null)}
          badge={<StatusPill status={editing.status || "published"} />}>
          <PostForm initial={editing} onSave={savePost} onCancel={() => setEditing(null)} onBump={bumpPost} loading={saving} />
        </Drawer>
      )}
      {creating && (
        <Drawer title="Новый пост" onClose={() => setCreating(false)}>
          <PostForm onSave={savePost} onCancel={() => setCreating(false)} loading={saving} />
        </Drawer>
      )}
    </div>
