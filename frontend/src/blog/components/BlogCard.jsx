...
// ── Section (rubric) badge ─────────────────────────────────────────────────

function SectionBadge({ sectionId, size = "sm" }) {
  const section = getSection(sectionId)
  if (!section) return null
  const pad = size === "sm" ? "px-1.5 py-0.5 text-[10px]" : "px-2 py-1 text-[11px]"
  return (
    <span
      className={`inline-flex items-center gap-1 font-futura font-black uppercase tracking-wider ${pad}`}
    ...
    >
      ...
      >
        ...
        <div className="flex-shrink-0 relative overflow-hidden bg-[#1a1a1a] w-full md:w-[200px] h-[200px] md:h-[134px]">
          ...
          {/* Rubric badge — overlay on thumbnail */}
          {post.section && (
            <div className="absolute top-1.5 left-1.5">
              <SectionBadge sectionId={post.section} />
            </div>
          )}
        </div>
        ...
        <div className="flex-shrink-0 relative overflow-hidden bg-[#1a1a1a] w-full md:w-[340px] h-[240px] md:h-[220px]">
          ...
          {post.section && (
            <div className="absolute top-2 left-2">
              <SectionBadge sectionId={post.section} size="md" />
            </div>
          )}
        </div>
