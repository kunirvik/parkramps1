...
// const SWIPE_THRESHOLD = 40; // px — минимальный горизонтальный сдвиг пальца, чтобы считать это свайпом
...
//   /* -------------------- SWIPE ПО СОДЕРЖИМОМУ ВКЛАДКИ (моб.) -------------------- */
...
//     // считаем жест горизонтальным, если по X сдвинулись заметно больше, чем по Y
...
//     const nextIndex =
//       dx < 0
//         ? Math.min(items.length - 1, currentIndex + 1) // свайп влево → следующая вкладка
//         : Math.max(0, currentIndex - 1); // свайп вправо → предыдущая вкладка

//     if (nextIndex === currentIndex) return;

//     setDirection(nextIndex > currentIndex ? 1 : -1);
//     // setOpenIndex здесь — это либо onToggle (controlled), либо внутренний стейт,
//     // так что для "вироби" сработает та же логика открытия галереи, что и по тапу
...
//           Свайп влево/вправо по этому блоку переключает вкладки (см. handleTabTouch*). */}
//       <div
...
const OVERFLOW_TOLERANCE = 20; // px — если текст выше клампа не более чем на это значение, кнопку не показываем
const SWIPE_THRESHOLD = 40; // px — минимальный горизонтальный сдвиг пальца, чтобы считать это свайпом
const TAB_SLIDE_OFFSET = 28; // px — на сколько новая вкладка "въезжает" сбоку
...
const Accordion = ({
  ...
  // ─── Слайд-эффект при смене активной вкладки (моб.) ────────────────────
  // При смене openIndex новый контент на первом кадре ставится сдвинутым
  // в сторону, откуда пришёл клик/свайп (по значению direction), а на
  // следующем кадре едет в translateX(0) — получается "въезд" вкладки,
  // а не просто мгновенная подмена + фейд.
  const [tabEntering, setTabEntering] = useState(false);
  const tabRafRef = useRef(null);

  useEffect(() => {
    if (isDesktop) return; // слайд нужен только на мобильных табах
    if (isFirstMount.current) return; // на монтировании въезжать не нужно

    setTabEntering(true);
    const raf1 = requestAnimationFrame(() => {
      const raf2 = requestAnimationFrame(() => setTabEntering(false));
      tabRafRef.current = raf2;
    });
    tabRafRef.current = raf1;

    return () => cancelAnimationFrame(tabRafRef.current);
        ...
        {items.map((item, index) => {
          ...
          return (
            <div
              key={index}
              style={{
                opacity: !contentVisible ? 0 : tabEntering ? 0 : 1,
                transform: tabEntering
                  ? `translateX(${direction * TAB_SLIDE_OFFSET}px)`
                  : "translateX(0)",
                transition: tabEntering
                  ? "none"
