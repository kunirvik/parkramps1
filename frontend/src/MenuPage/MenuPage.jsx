...
const HERO_ROTATION = (72 * Math.PI) / 180;

export default function MenuPage() {
  const [index, setIndex] = useState(0);
  const navigate = useNavigate();
  const { showHero, flyToLogoAndNavigate } = useSceneTransition();
  ...
  // Показываем геройскую модель в персистентном слое, когда страница
  // смонтирована. Фон под ней всё ещё рисует сама MenuPage (картинка
  // и заголовки), 3D-модель просто "плавает" поверх.
  useEffect(() => {
    showHero(heroModelUrl, { size: 4.5, rotation: HERO_ROTATION, media: background });
  }, [showHero]);
  ...
  const handleExplore = () => {
    ...
    // модель сжимается, сереет и "летит" на место логотипа в шапке,
    // и только после этого происходит переход на /catalogue —
    // общий канвас при этом не размонтируется
    flyToLogoAndNavigate({
      toUrl: logoModelUrl,
