...
// ============================================================
// LOAD MODEL (возвращает Promise<Object3D>)
// ============================================================

function loadModel({ url, fallbackGeo, material, label = "model" }) {
  return new Promise((resolve) => {
    if (!url) {
      resolve(new THREE.Mesh(fallbackGeo, material));
      return;
    }
    ...
    loader.load(
      ...
      undefined,
      (err) => {
        console.error(`[Hero3D] Не удалось загрузить ${label} (${url}):`, err);
        resolve(new THREE.Mesh(fallbackGeo, material));
      }
    );
  });
...
function fitAndCenter(object, targetSize) {
  ...
  // возвращаем "целевой" множитель масштаба, чтобы потом
  // можно было анимировать scale от 0 до targetScale
  return targetSize / maxDim;
}

// ============================================================
// ШЕЙДЕРНЫЙ МАТЕРИАЛ (с добавленным uOpacity для кроссфейда)
// ============================================================
...
function makeBgSampleMaterial(mediaTexture) {
  const material = new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uMediaNative: { value: new THREE.Vector2(16, 9) },
    ...
    fragmentShader: `
      ...
      vec3 applyRelief(vec3 flatNormal) {
        ...
        vec2 grad = vec2(nx - n, ny - n) / eps;
        vec3 bumped = flatNormal + vec3(grad * uReliefStrength, 0.0);
        return normalize(bumped);
      }

      vec2 coverUV(vec2 screenUV) {
        float containerAspect = uContainerSize.x / uContainerSize.y;
        float mediaAspect = uMediaNative.x / uMediaNative.y;
        vec2 uv = screenUV;

        if (containerAspect > mediaAspect) {
          float scale = mediaAspect / containerAspect;
          uv.y = (uv.y - 0.5) * scale + 0.5;
        } else {
          float scale = containerAspect / mediaAspect;
      ...
      vec3 sampleBg(vec2 local, vec3 flatNormal) {
        ...
        vec2 mediaUV = coverUV(clamp(distorted, 0.0, 1.0));
        vec2 sampleUV = vec2(mediaUV.x, 1.0 - mediaUV.y);
        return texture2D(uMediaTex, sampleUV).rgb;
      }
      ...
      void main() {
        ...
        vec3 reliefNormal = applyRelief(flatNormal);
        ...
        float r = sampleBg(local, reliefNormal + vec3(uChromaShift, 0.0, 0.0)).r;
        float g = sampleBg(local, reliefNormal).g;
        float b = sampleBg(local, reliefNormal - vec3(uChromaShift, 0.0, 0.0)).b;

        vec3 bg = vec3(r, g, b);

        float fresnel = pow(
          1.0 - max(dot(normalize(vViewDir), reliefNormal), 0.0),
          1.6
        );
...
export default function Hero3D({
  ...
  grayscale = false,
  // Если задано — канвас всегда рендерится с этим фиксированным
  // пиксельным размером, игнорируя авто-расчёт от viewport. Нужно для
  // сценария "докинга": снаружи модель масштабируется CSS-transform'ом
  // обёртки, а сам WebGL-канвас должен быть неизменного размера, чтобы
  // расчёт scale снаружи был предсказуемым.
  canvasSizePx = null,
}) {
  const mountRef = useRef(null);
  const heroSectionRef = useRef(null);

  // живые значения пропсов — animate() всегда видит актуальные
  ...
  const threeRef = useRef({
    ...
    slots: {
      A: { object: null, material: null, url: null, targetScale: 1 },
      B: { object: null, material: null, url: null, targetScale: 1 },
    },
    activeKey: "A",
  ...
  useEffect(() => {
    ...
    const heroEl = mount.closest(".hero3d");
    ...
    const getSize = () => {
      if (canvasSizePx) return canvasSizePx;

      const vw = window.innerWidth;
      const vh = window.innerHeight;
      const base = Math.min(vw, vh) * CONFIG.viewportFraction;
      return Math.round(
        Math.max(CONFIG.minCanvasPx, Math.min(base, CONFIG.maxCanvasPx))
      );
    };
    ...
    // создаём материалы для обоих слотов сразу
    const materialA = makeBgSampleMaterial(new THREE.Texture());
    const materialB = makeBgSampleMaterial(new THREE.Texture());
    materialB.uniforms.uOpacity.value = 0; // B изначально невидим

    threeRef.current.scene = scene;
    threeRef.current.camera = camera;
    threeRef.current.renderer = renderer;
    threeRef.current.slots.A.material = materialA;
    threeRef.current.slots.B.material = materialB;
    ...
    const updateScreenUniforms = () => {
      ...
      [materialA, materialB].forEach((mat) => {
        mat.uniforms.uDPR.value = dpr;
        mat.uniforms.uContainerSize.value.set(rect.width, rect.height);
        mat.uniforms.uContainerOffset.value.set(rect.left, rect.top);
        mat.uniforms.uCanvasOffset.value.set(canvasRect.left, canvasRect.top);
        mat.uniforms.uCanvasSize.value.set(canvasRect.width, canvasRect.height);
      });
    };
    ...
    // ========================================================
    // ПОКАЗАТЬ АКТИВНУЮ МОДЕЛЬ, КОГДА ОНА + МЕДИА ГОТОВЫ
    // ========================================================

    const tryRevealActive = () => {
      const t = threeRef.current;
      const slot = t.slots[t.activeKey];

      if (slot.object && t.mediaReady && !slot.object.visible) {
        slot.object.visible = true;

        if (t.modelRevealedAt === null) {
          t.modelRevealedAt = performance.now();
        }
      }
    };

    threeRef.current.tryRevealActive = tryRevealActive;

    // ========================================================
    // POINTER CONTROL (работает только с активным слотом)
    // ========================================================
    ...
    const getActiveObject = () =>
      threeRef.current.slots[threeRef.current.activeKey].object;
    ...
    const onPointerMove = (event) => {
      const obj = getActiveObject();
      if (!state.dragging || !obj || event.pointerId !== state.activePointerId) {
        return;
      }
    ...
    const animate = () => {
      ...
      if (obj && !t.isTransitioning) {
        if (state.dragging) {
          ...
          const revealedAt = t.modelRevealedAt;
          const withinInitialPause =
            revealedAt === null ||
            performance.now() - revealedAt < CONFIG.autoRotateDelayMs;

          if (!withinInitialPause) {
            const scrolledRecently =
              performance.now() - scrollState.lastScrollAt <
              CONFIG.scrollPauseMs;
          ...
          if (Math.abs(diff) > 0.002) {
            ...
            obj.rotation.y = propsRef.current.restRotationY;
            state.settling = false;
            state.velocity = 0;
            state.hasInteracted = false;
          }
    ...
    // ========================================================
    // CLEANUP (только при полном размонтировании компонента)
    // ========================================================
    ...
    return () => {
      ...
      gsap.killTweensOf("*"); // на всякий случай гасим активные твины этого компонента

      Object.values(threeRef.current.slots).forEach((slot) => {
        if (slot.object) {
  ...
  // материалам сразу, чтобы фон был одинаковым у обеих моделей.
  // ==========================================================

  useEffect(() => {
    const t = threeRef.current;
    if (!t.slots.A.material || !t.slots.B.material) return;

    const mediaType = media && media.type;
    const mediaUrl = media && media.url;
    const videoEl = mediaType === "video" ? (videoRef && videoRef.current) : null;
    ...
    const applyTexture = (tex, nativeW, nativeH) => {
      ...
      [t.slots.A.material, t.slots.B.material].forEach((mat) => {
        mat.uniforms.uMediaTex.value = tex;
        if (nativeW && nativeH) {
          mat.uniforms.uMediaNative.value.set(nativeW, nativeH);
        }
      });
    ...
    if (mediaType === "video" && videoEl) {
      ...
      if (videoEl.readyState >= 2) {
        applyTexture(newTexture, videoEl.videoWidth, videoEl.videoHeight);
      } else {
        onLoadedData = () =>
          applyTexture(newTexture, videoEl.videoWidth, videoEl.videoHeight);
      ...
      onLoadedMetadata = () => {
        [t.slots.A.material, t.slots.B.material].forEach((mat) => {
          mat.uniforms.uMediaNative.value.set(videoEl.videoWidth, videoEl.videoHeight);
        });
      };
      ...
      newTexture = textureLoader.load(
        ...
        undefined,
        (err) => {
          console.error(`[Hero3D] Не удалось загрузить фото (${mediaUrl}):`, err);
          applyTexture(newTexture);
        }
      );
      ...
      console.warn("[Hero3D] media не задан или некорректен.");
    }
    ...
    return () => {
      ...
      if (mediaType === "video" && videoEl) {
        ...
        if (onLoadedMetadata) videoEl.removeEventListener("loadedmetadata", onLoadedMetadata);
      }
  ...
  // ==========================================================
  // ЭФФЕКТ №3 — СМЕНА МОДЕЛИ (чёткий, последовательный переход
  // между слотами: старая модель полностью исчезает, и ТОЛЬКО
  // ПОСЛЕ ЭТОГО появляется новая — без наложения прозрачностей
  // друг на друга, и без вращения во время фейда).
  // ==========================================================

  useEffect(() => {
    const t = threeRef.current;
    if (!t.scene) return; // сцена ещё не готова (эффект №1 не отработал)
    ...
    const run = async () => {
      ...
      const activeSlot = t.slots[activeKey];
      const inactiveSlot = t.slots[inactiveKey];

      // если этот url уже отображается активным слотом — ничего не делаем
      if (activeSlot.url === modelUrl) return;

      // грузим новую модель в неактивный слот
      const object = await loadModel({
        url: modelUrl,
        fallbackGeo: new THREE.IcosahedronGeometry(1, 2),
        material: inactiveSlot.material,
        label: "modelUrl",
      });

      if (cancelled) return;

      const targetScale = fitAndCenter(object, modelSize);
      inactiveSlot.targetScale = targetScale;
      inactiveSlot.url = modelUrl;

      // убираем старый объект слота (если был) из сцены
      if (inactiveSlot.object) {
        t.scene.remove(inactiveSlot.object);
        inactiveSlot.object.traverse((child) => {
          if (child.isMesh && child.geometry) child.geometry.dispose();
        });
      }

      // ВАЖНО: если это не первая загрузка (т.е. идёт переход между
      // моделями), новая модель стартует с ТЕКУЩИМ углом поворота
      // старой модели, а не с "дефолтного" restRotationY. Иначе новая
      // модель на миг видна неповёрнутой в "дефолтной позе" — именно
      // это вызывало эффект "модель появляется большая, не повёрнута".
      object.rotation.y = activeSlot.object
        ? activeSlot.object.rotation.y
        : restRotationY;
      inactiveSlot.object = object;
      t.scene.add(object);

      const isFirstLoad = !activeSlot.object;
      ...
      // ---------- ЧЁТКИЙ ПОСЛЕДОВАТЕЛЬНЫЙ ПЕРЕХОД ----------
      // Вместо одновременного fade-out старой и fade-in новой
      // (что давало эффект "двойной экспозиции" двух
      // полупрозрачных моделей одновременно), делаем это
      // последовательно: сначала старая полностью гаснет,
      // потом появляется новая. Плюс убираем overshoot
      // (back.out) в пользу чистого power3.out.

      t.isTransitioning = true;

      const fullDuration = transitionDuration;
      const outDuration = fullDuration * 0.42;
      const inDuration = fullDuration * 0.58;

      object.visible = true;
      object.scale.setScalar(0.0001);
      inactiveSlot.material.uniforms.uOpacity.value = 0;

      // на время перехода слегка приглушаем дисторсию/fresnel
      // нового материала — так контур модели читается чётче,
      // без "плавающих" краёв на полупрозрачности
      const newMatUniforms = inactiveSlot.material.uniforms;
      const baseDistortion = newMatUniforms.uDistortion.value;
      const baseFresnel = newMatUniforms.uFresnelStrength.value;
      newMatUniforms.uDistortion.value = 0;
      newMatUniforms.uFresnelStrength.value = baseFresnel * 0.3;

      const tl = gsap.timeline({
        onComplete: () => {
          // старая модель больше не нужна — освобождаем память
          t.scene.remove(activeSlot.object);
          activeSlot.object.traverse((child) => {
            if (child.isMesh && child.geometry) child.geometry.dispose();
          });
          activeSlot.object = null;
          activeSlot.url = null;

          // возвращаем полноценную дисторсию/fresnel новому материалу
          newMatUniforms.uDistortion.value = baseDistortion;
          newMatUniforms.uFresnelStrength.value = baseFresnel;
      ...
      // Этап 1 (0 → outDuration): старая модель уходит —
      // гаснет и сжимается. Новая пока полностью невидима.
      tl.to(
        activeSlot.material.uniforms.uOpacity,
        { value: 0, duration: outDuration, ease: "power2.in" },
        0
      );
      tl.to(
        activeSlot.object.scale,
        {
          x: 0.0001,
          y: 0.0001,
          z: 0.0001,
      ...
      // Этап 2 (outDuration → конец): старая уже погасла,
      // теперь появляется новая — без нахлёста прозрачностей
      // и без пружинного overshoot (используем power3.out).
      tl.to(
        newMatUniforms.uOpacity,
        { value: 1, duration: inDuration, ease: "power2.out" },
        outDuration
      );
      tl.to(
        object.scale,
...
//       object.rotation.y = restRotationY;
//       inactiveSlot.object = object;
//       t.scene.add(object);
...
//       uReliefStrength: { value: 0.1 },
//       uReliefScale: { value: 6.0 },
//       // НОВОЕ: прозрачность для кроссфейда
//       uOpacity: { value: 1.0 },
//     },
...
//   // делается кроссфейдом, без пересоздания сцены/рендерера.
//   // ==========================================================
...
//         }
//       } else if (obj && t.isTransitioning) {
//         // во время перехода модель продолжает медленно крутиться,
//         // чтобы кроссфейд не выглядел статично
...
//   // ==========================================================
//   // ЭФФЕКТ №3 — СМЕНА МОДЕЛИ (кроссфейд между слотами)
//   // ==========================================================
...
//       // новая модель: появляется и увеличивается
//       tl.to(
//         inactiveSlot.material.uniforms.uOpacity,
//         { value: 1, duration: transitionDuration, ease: "power2.out" },
//         0
//       );
//       tl.to(
//         object.scale,
...
//       // старая модель: исчезает и уменьшается
//       tl.to(
//         activeSlot.material.uniforms.uOpacity,
//         { value: 0, duration: transitionDuration * 0.8, ease: "power2.in" },
//         0
//       );
//       tl.to(
//         activeSlot.object.scale,
