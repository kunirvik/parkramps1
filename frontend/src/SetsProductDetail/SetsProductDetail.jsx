...
export default function SetsProductDetail() {
  ...
  const updateUrl = useCallback((productId) => {
    ...
    refs.current.urlUpdateBlocked = true;
    window.history.replaceState(null, "", `/product/sets/${productId}`);
    setTimeout(() => { refs.current.urlUpdateBlocked = false; }, 50);
  }, []);
  ...
  const clearImageDataFromHistory = useCallback(() => {
    const currentState = window.history.state || {};
    window.history.replaceState(
      { ...currentState, usr: { ...(currentState.usr || {}), imageData: null } },
      "",
      window.location.pathname + window.location.search
    );
  }, []);
  ...
  const handleSwiperInit = useCallback(
    (swiper) => {
      ...
      if (!imageData) {
        if (!state.thumbsShown) {
          ...
          gsap.set(refs.current.thumbs, { opacity: 0, y: 20 });
          showInfoAndThumbs().then(() =>
            updateState({ thumbsShown: true, purchaseShown: true, productionShown: true })
          );
        }
        return;
      }
    ...
    [imageData, startTransitionAnimation, state.thumbsShown, showInfoAndThumbs, updateState]
  );

  // ─── Slide change — фикс лага ─────────────────────────────────────────────
  const handleSlideChange = useCallback(
    async (swiper) => {
      socialButtonsRef.current?.close();
      const newIndex = swiper.activeIndex;
      ...
      // Немедленно стопаем анимацию и сбрасываем кадр старого продукта
      stopHoverAnimation();
      setState((prev) => {
        const newIndices = [...prev.selectedImageIndices];
        newIndices[oldIndex] = 0; // сразу сбрасываем кадр предыдущего
        return { ...prev, selectedImageIndices: newIndices };
      });
      ...
      if (isMobile) {
        ...
        setAccordionState((prev) => ({ purchase: null, product: 0, virobi: prev.virobi }));
      }
      ...
      setState((prev) => {
        const newIndices = [...prev.selectedImageIndices];
        newIndices[newIndex] = 0;
        return { ...prev, activeProductIndex: newIndex, selectedImageIndices: newIndices };
      });
  ...
  const handleThumbnailClick = useCallback(
    (index) => {
      socialButtonsRef.current?.close();
      if (animationInProgressRef.current || index === state.activeProductIndex || !swiperInstances.main) return;
      swiperInstances.main.slideTo(index);
    },
    [state.activeProductIndex, swiperInstances.main]
  );
  ...
  useEffect(() => {
    ...
    const applyStyles = () => {
      styleElement.innerHTML = `
        html, body { overflow: auto !important; height: 100% !important; width: 100% !important; }
        ...
        .swiper-no-transition .swiper-wrapper { transition: none !important; }
        .swiper-slide-thumb-active { opacity: 1 !important; transform: scale(1.05) !important; }
        .transition-image-container {
  ...
  }, [swiperInstances.main?.activeIndex, state.activeProductIndex, updateState, updateUrl, swiperInstances.thumbs]);

  // ─── Early returns ────────────────────────────────────────────────────────
  if (!currentProduct) return <div className="text-center mt-10 p-4">Продукт не найден</div>;
  if (loadingState.isLoading) return <LoadingScreen onComplete={handleLoadingComplete} />;

  // ─── Render ───────────────────────────────────────────────────────────────
  return (
    <>
      <div className="flex flex-col bg-center h-screen overflow-hidden bg-no-repeat bg-[url('https://res.cloudinary.com/dbx6muxub/image/upload/v1780563482/project-brightness-50_fbitrl.png')]">
        <div className="z-50 flex-shrink-0">
          <SocialButtons
            ref={socialButtonsRef}
            buttonLabel="shop"
        ...
        <div
          ref={(el) => (refs.current.container = el)}
          className="w-full flex-grow mt-[1px] lg:mt-[50px] px-4"
          style={{ opacity: shouldShowLoading && !loadingState.isCompleted ? 0 : 1 }}
        >
          <div className="w-full hidden sm:block flex items-start mb-4" />
