...
// Глобальный флаг — подсказка была показана хоть раз за сессию
const hintShownOnce = { play: false };
...
export default function ProductGallery({
  ...
  // Подсказка — только один раз за сессию
  useEffect(() => {
    if (!hintShownOnce.play) {
      hintShownOnce.play = true;
      setAutoHintVisible(true);
      clearTimeout(autoHintTimerRef.current);
      autoHintTimerRef.current = setTimeout(() => setAutoHintVisible(false), 2800);
    }
  ...
  }, [allImages.length, isPlaying, stopHoverAnimation, startPlayAnimation, state.activeProductIndex, currentProduct]);
  ...
  const cursorIconClass = isPlaying ? "ti ti-player-pause" : "ti ti-player-play";
    ...
    >
      <div className="w-full flex flex-col">
        ...
        <div
          ...
          onMouseMove={!isTouchDevice ? handleStageMouseMove : undefined}
        ...
        >
          ...
          {allImages.length > 1 && !isTouchDevice && isHovering && (
            <div
              className="absolute pointer-events-none z-20"
              style={{
                left: cursorPos.x,
          ...
          {allImages.length > 1 && isTouchDevice && (
            <button
              ...
              onClick={(e) => { e.stopPropagation(); handleTouchTap(); }}
