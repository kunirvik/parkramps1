...
// Filmstrip (прогресс-бар кадров)
function Filmstrip({ product, currentFrameIndex }) {
  const allImages = product ? [product.image, ...(product.altImages || [])] : [];
  if (allImages.length <= 1) return null;

  return (
    <div className="w-full mt-3 select-none">
      <div className="w-full h-[2px] bg-gray-200 rounded overflow-hidden">
        <div
...
export default function ProductGallery({
  ...
  const [isPlaying, setIsPlaying] = useState(false);
  const [isHovering, setIsHovering] = useState(false);
  // Подсказка показывается ровно 1 раз при первой загрузке
  const [hintVisible, setHintVisible] = useState(true);
  const hintTimerRef = useRef(null);
  const stageRef = useRef(null);

  // Показываем подсказку 1 раз при монтировании, скрываем через 3 сек
  useEffect(() => {
    hintTimerRef.current = setTimeout(() => setHintVisible(false), 3000);
    return () => clearTimeout(hintTimerRef.current);
  }, []);

  // При смене продукта — стоп анимации
  useEffect(() => {
    setIsPlaying(false);
    stopHoverAnimation();
  ...
  }, [hasMultiple, isPlaying, stopHoverAnimation, startPlayAnimation, state.activeProductIndex, currentProduct]);

  const cursorIcon = isPlaying ? "ti ti-player-pause" : "ti ti-player-play";
      ...
      <div className="w-full flex flex-col">
              ...
              return (
                <SwiperSlide key={product.id} className="!w-[120px] sm:!w-[140px] lg:!w-[200px]">
                  <img
                    ...
                    className={`transition-all duration-300 rounded-lg px-3 w-full h-20 sm:h-24 lg:h-28 object-contain
                      ${isActive
                        ? "opacity-100 scale-105 cursor-default ring-1 ring-white/40"
                        : "grayscale opacity-60 hover:opacity-100 cursor-pointer"
                      }`}
                    alt={product.name}
                    draggable="false"
        ...
        >
          ...
          {hasMultiple && (
            <div
              ...
              style={{
                ...
                transform: hintVisible ? "translateY(0)" : "translateY(-4px)",
                transition: "opacity 0.3s, transform 0.3s",
              }}
            >
              <p style={{ fontSize: 12, color: "rgba(255,255,255,0.88)", lineHeight: 1.5 }}>
                Натисніть на фото — запуститься анімація. Ще раз — пауза
              </p>
            </div>
          )}
          ...
          {hasMultiple && isHovering && (
            ...
            >
              <i className={cursorIcon} style={{ fontSize: 15, color: "#fff" }} aria-hidden="true" />
            </div>
          )}
        </div>

        {/* Filmstrip (прогресс кадров) */}
        <Filmstrip
          product={currentProduct}
          currentFrameIndex={currentFrameIndex}
        />
